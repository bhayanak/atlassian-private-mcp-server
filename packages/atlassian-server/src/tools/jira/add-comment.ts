...
export type AddCommentToJiraIssueInput = z.infer<typeof addCommentToJiraIssueSchema>;
...
export async function addCommentToJiraIssue(
  ...
  const result = await client.post<{
    ...
    created: string;
  }>(`/rest/api/2/issue/${encodeURIComponent(input.issueIdOrKey)}/comment`, payload);
