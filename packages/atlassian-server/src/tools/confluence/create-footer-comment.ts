...
export const createConfluenceFooterCommentSchema = z.object({
  ...
  body: z
    .string()
    .describe("Comment body in Confluence Storage Format or wiki markup"),
  bodyFormat: z
    .enum(["storage", "wiki"])
    .optional()
  ...
  parentCommentId: z
    ...
    .optional()
    .describe(
      "Parent comment ID to create a reply (omit for top-level comment)"
    ),
});
...
export async function createConfluenceFooterComment(
  ...
  const comment = await client.post<ConfluencePage>(
    `/rest/api/content`,
    payload
  );
