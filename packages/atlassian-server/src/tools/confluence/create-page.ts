...
export const createConfluencePageSchema = z.object({
  ...
  body: z.string().describe('Page body content in Confluence Storage Format (XHTML) or plain text'),
  bodyFormat: z
    .enum(['storage', 'wiki', 'plain'])
    .optional()
    .describe("Body format: 'storage' (XHTML, default), 'wiki' (wiki markup), 'plain'"),
  parentPageId: z.string().optional().describe('Parent page ID to create this page as a child'),
  status: z
    .enum(['current', 'draft'])
    .optional()
...
export type CreateConfluencePageInput = z.infer<typeof createConfluencePageSchema>;
...
export async function createConfluencePage(
  ...
  if (input.labels && input.labels.length > 0) {
    ...
    await client.post(`/rest/api/content/${page.id}/label`, labelPayload);
  }
