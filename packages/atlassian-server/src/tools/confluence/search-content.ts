...
export const searchConfluenceUsingCqlSchema = z.object({
  ...
  expand: z
    ...
    .optional()
    .describe(
      "Properties to expand on results, e.g. ['version', 'space', 'body.view']"
    ),
});

export type SearchConfluenceUsingCqlInput = z.infer<
  typeof searchConfluenceUsingCqlSchema
>;
...
export async function searchConfluenceUsingCql(
  ...
  const expand =
    input.expand?.join(",") || "version,space,metadata.labels";
